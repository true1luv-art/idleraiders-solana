 ...
 *
 * NOTE: `amount` is intentionally NOT stored here. Base drop amounts are
 * static config (always 1 in Phase 2, later scaled by skill level bonuses).
 * Storing them in MongoDB would mean touching every document whenever the
 * value changes. The server injects `amount: 1` at hydration time instead.
 */
...
export interface IFarm extends Document {
  ...
  // Lifetime counters — used for milestone tracking. §2.5-C
