...
class ScreenTracker {
  ...
  /**
   * Reset the collinearity counter and movement history.
   * Call this before a game action that is triggered by a trusted source
   * (e.g. the Phaser canvas) so the bot-detection does not accumulate
   * across clicks that naturally produce an empty movement buffer.
   */
  public reset() {
    this.movements = [];
    this.tracks = 0;
