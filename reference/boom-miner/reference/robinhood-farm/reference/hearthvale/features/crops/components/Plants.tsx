...
export const Plants: React.FC = () => {
  const [selected, setSelected] = useState<Crop>(CROPS().Potato);
  ...
  const sell = (amount = 1) => {
    ...
    setToast({
      content: "VTC +$" + displaySellPrice(selected).mul(amount).toString(),
    });
  };

  const cropAmount = new Decimal(inventory[selected.name] || 0).toNumber();
  const noCrop = cropAmount === 0;
        ...
        <div className="flex flex-col justify-center items-center p-2 ">
          ...
          <div className="border-t border-white w-full mt-2 pt-1">
            <div className="flex justify-center items-end">
              ...
              {isPriceBoosted && (
                <img src={typeof lightning === "string" ? lightning : lightning?.src} className="h-6 me-2" />
              )}
              <span className="text-xs text-shadow text-center mt-2 ">
                {`$${displaySellPrice(selected).toString()}`}
              </span>
            </div>
          </div>
          <Button
            disabled={cropAmount < 1}
