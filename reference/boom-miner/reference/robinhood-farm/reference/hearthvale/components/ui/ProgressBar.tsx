...
export const Bar: React.FC<Props> = ({ percentage }) => {
  if (percentage >= 100) {
    return <img src={typeof progressFull === "string" ? progressFull : progressFull?.src} className="w-10" />;
  }
  if (percentage >= 75) {
  ...
  if (percentage >= 25) {
    return <img src={typeof progressQuarter === "string" ? progressQuarter : progressQuarter?.src} className="w-10" />;
  }

  return <img src={typeof progressStart === "string" ? progressStart : progressStart?.src} className="w-10" />;
};
