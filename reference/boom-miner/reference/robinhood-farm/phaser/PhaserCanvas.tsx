  ...
  useEffect(() => {
    const onCaught = () => {
      ...
      try {
        ...
        const { fishing, inventory } = useGameStore.getState().state;
        if (fishing.lastCaughtFish) {
          const fish = fishing.lastCaughtFish;
          const prev = Number(prevState.inventory[fish] ?? 0);
          const next = Number(inventory[fish] ?? 0);
...
export default function PhaserCanvas() {
  ...
  if (typeof window !== "undefined") {
    ...
    w.__playerCoins    = player?.coins    ?? 0;
        ...
        <ToastQueueProvider>
          ...
          <div className="relative w-full h-full">
            ...
            <div
              ...
              aria-label="Robinhood Farm town — game canvas"
            />
            ...
            {!farmReady && (
              <div
                role="status"
                aria-label="Loading Robinhood Farm"
