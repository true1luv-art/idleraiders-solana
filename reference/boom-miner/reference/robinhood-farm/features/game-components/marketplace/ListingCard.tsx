...
export function ListingCard({ listing, onSelect, isMine, onCancel, cancelling }: ListingCardProps) {
  return (
    <article
      className="group relative flex flex-col border-2 border-border bg-card transition-all duration-150 cursor-pointer hover:border-primary/60 hover:shadow-[0_2px_0_0_hsl(var(--primary)/0.4)]"
    ...
    >
      ...
      <div className="px-3 pb-2">
        <p className="font-pixel text-[10px] text-gold leading-relaxed">
          ...
          <span className="text-[7px] text-muted-foreground ml-1">/ unit</span>
        </p>
        {listing.quantity > 1 && (
          <p className="text-[9px] font-sans text-muted-foreground mt-0.5">
            Total: {fmtPrice(listing.price * listing.quantity)} coins
          </p>
        )}
      </div>
      ...
      <div className="px-3 pb-3 flex gap-2">
        <button
          type="button"
          className="flex-1 border-2 border-primary bg-primary text-primary-foreground font-pixel text-[8px] uppercase py-1.5 transition-all hover:brightness-110 active:brightness-90"
