 ...
 *
 * NOTE: `amount` is NOT written back to MongoDB — it is static config injected at
 * hydration time and must never live in the document.
 */
function resourceNodesDiff(
  ...
  for (const [idx, node] of Object.entries(newNodes ?? {})) {
    ...
    if (!old || phaserHarvested !== oldHarvested) {
      set[`${collection}.${idx}`] = {
        name:        node.name,
...
export async function persistFarmChanges(
  ...
  const fishingChanged =
    ...
    oldState.fishing.lastCaughtFish !== newState.fishing.lastCaughtFish;

  // (cooking removed — no longer tracked in GameState)
  ...
  try {
    ...
    // Player-doc changes: skill XP ($inc) and persisted Game Balance (HFARM).
    if (hasPlayerChanges) {
      const playerUpdate: Record<string, unknown> = {};
      if (hasSkillChanges) playerUpdate["$inc"] = xpDeltas;
      if (hasBalanceChange) {
        const player = await PlayerModel.findOne({ wallet: playerId }, { coins: 1 }, opts).lean<{
          coins: number;
        }>();
        playerUpdate["$set"] = {
          coins: Math.max(0, (player?.coins ?? 0) + balDelta),
        };
      }
