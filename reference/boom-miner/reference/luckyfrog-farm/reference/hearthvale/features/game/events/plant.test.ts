...
describe("plant", () => {
  ...
  it("applies Double Harvest skill to crop yield multiplier", () => {
    const state = plant({
      state: {
        ...GAME_STATE,
        inventory: {
          "Potato Seed": new Decimal(5),
          "Double Harvest": new Decimal(1), // Skill purchased
        },
      },
      action: {
        type: "item.planted",
    ...
    // Multiplier should be 1.15 with Double Harvest skill (+15% chance)
    expect(state.fields[0].multiplier).toBeCloseTo(1.15, 2);
  });

  it("applies Green Thumb skill for faster crop growth", () => {
    const now = Date.now();
    const state = plant({
      state: {
        ...GAME_STATE,
        inventory: {
          "Potato Seed": new Decimal(5),
          "Green Thumb": new Decimal(1), // 20% faster growth
        },
      },
      action: {
        type: "item.planted",
    ...
    // Green Thumb: 60 * 0.8 = 48 seconds base time
    // Offset = 60 - 48 = 12 seconds
    const expectedOffset = 60 - 60 * 0.8;
    const actualOffset = (now - state.fields[0].plantedAt) / 1000;
