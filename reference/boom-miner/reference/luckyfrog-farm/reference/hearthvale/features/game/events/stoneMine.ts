...
export function canMine(rock: GameNode, now: number = Date.now()) {
  return now - (rock.minedAt ?? 0) > STONE_RECOVERY_TIME * 1000;
}

export function mineStone({ state, action, createdAt = Date.now() }: Options): GameState {
  if (!hasEnoughStamina(state.stamina.current, "mine_stone")) {
  ...
  const stoneDrop = isDouble ? boosted * 2 : boosted;

  const amount = new Decimal(state.inventory.Stone || 0);

  // Route XP to mining skill
  const mineXP      = getSkillXP("mine_stone");
  ...
  const activity = trackActivity(state.activity, "Stone Mined", 1);

  return {
    ...state,
    inventory: {
      ...state.inventory,
      Stone: amount.add(stoneDrop),
    },
    stones: {
      ...state.stones,
      [action.index]: { name: "Stone" as const, minedAt: Date.now(), amount: 2 },
    },
    skills: { ...state.skills, mining: newMiningXP },
    bonus:  newBonus,
    stamina: {
      ...state.stamina,
      current: deductStamina(state.stamina.current, "mine_stone"),
    },
    activity,
  };
}
