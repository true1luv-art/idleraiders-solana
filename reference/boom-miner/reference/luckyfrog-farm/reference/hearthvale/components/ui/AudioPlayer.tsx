    ...
    >
        ...
        <div className="p-1 sm:mr-2 relative">
          ...
          <div className="flex space-x-2 justify-content-between ">
            <Button onClick={handlePlayState} className="w-10 h-8">
              <img src={getImageSrc(isPlaying ? pause : play)} alt="play / pause button " />
            </Button>
            <Button onClick={handleNextSong} className="w-10 h-8">
              <img src={typeof skip_forward === "string" ? skip_forward : skip_forward?.src} alt="next song button" />
            </Button>
            {/*Custom volume range */}
            <Button
            ...
            >
              <img src={typeof volume_down === "string" ? volume_down : volume_down?.src} alt="next song button" />
            </Button>
            <Button
              onClick={volume.increase}
              className="w-10 h-8 hidden sm:flex"
            >
              <img src={typeof volume_up === "string" ? volume_up : volume_up?.src} alt="next song button" />
            </Button>
      ...
      >
        <Button onClick={() => setIsVisible(!visible)}>
          <img
            src={getImageSrc(visible ? chevron_right : music_note)}
